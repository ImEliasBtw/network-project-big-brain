---
grupo: 4
dominio: redes4-5TX.com.ni
ip_servidor: 192.168.28.98
gateway: 192.168.28.97
red: 192.168.28.96/27
mascara: 255.255.255.224
dhcp_inicio: 192.168.28.99
dhcp_fin: 192.168.28.126
broadcast: 192.168.28.127
distro: por detectar
---

# Configuracion del servidor — Grupo 4

Eres un agente de configuracion de sistemas Linux. Configura este servidor como
infraestructura de red para el **Grupo 4** de la seccion 5TX de Redes de
...
| Mascara         | 255.255.255.224 |
| Gateway         | 192.168.28.97 |
| IP del servidor | 192.168.28.98 |
| Rango DHCP      | 192.168.28.99 — 192.168.28.126 |
| Broadcast       | 192.168.28.127 |
| Hostnames DNS   | ns / www / ftp / mail . redes4-5TX.com.ni |
...
```bash
sudo nmcli con mod "CONEXION" ipv4.addresses 192.168.28.98/27
sudo nmcli con mod "CONEXION" ipv4.gateway 192.168.28.97
sudo nmcli con mod "CONEXION" ipv4.dns 127.0.0.1
sudo nmcli con mod "CONEXION" ipv4.method manual
sudo nmcli con up "CONEXION"
```

**Verificacion:** `ip a` debe mostrar `192.168.28.98/27` en la interfaz.
...
```
option domain-name "redes4-5TX.com.ni";
option domain-name-servers 192.168.28.98;
default-lease-time 600;
max-lease-time 7200;
authoritative;

subnet 192.168.28.96 netmask 255.255.255.224 {
  range 192.168.28.99 192.168.28.126;
  option routers 192.168.28.97;
  option subnet-mask 255.255.255.224;
  option broadcast-address 192.168.28.127;
  option domain-name-servers 192.168.28.98;
  option domain-name "redes4-5TX.com.ni";
}
```
...
```
zone "redes4-5TX.com.ni" {
    type master;
    file "/etc/bind/db.redes4-5TX.com.ni";
};
...
Zona directa `/etc/bind/db.redes4-5TX.com.ni`:

```
$TTL 604800
@   IN  SOA ns.redes4-5TX.com.ni. admin.redes4-5TX.com.ni. (
            2026061801 ; Serial
            604800     ; Refresh
            86400      ; Retry
            2419200    ; Expire
...
;
@       IN  NS  ns.redes4-5TX.com.ni.
@       IN  MX  10 mail.redes4-5TX.com.ni.
ns      IN  A   192.168.28.98
www     IN  A   192.168.28.98
ftp     IN  A   192.168.28.98
mail    IN  A   192.168.28.98
```
...
;
@       IN  NS  ns.redes4-5TX.com.ni.
98      IN  PTR ns.redes4-5TX.com.ni.
98      IN  PTR www.redes4-5TX.com.ni.
98      IN  PTR mail.redes4-5TX.com.ni.
```
...
sudo named-checkzone redes4-5TX.com.ni /etc/bind/db.redes4-5TX.com.ni
sudo systemctl enable --now bind9
```

**RHEL:** el paquete es `bind bind-utils`, el demonio `named`, la config
`/etc/named.conf` y las zonas van en `/var/named/` (`db.redes4-5TX.com.ni.zone` y
...
**Verificacion:** `nslookup www.redes4-5TX.com.ni 127.0.0.1` debe devolver `192.168.28.98`.
...
VirtualHost `/etc/apache2/sites-available/grupo-4.conf`:

```
<VirtualHost *:80>
    ServerName www.redes4-5TX.com.ni
    ServerAlias redes4-5TX.com.ni
    DocumentRoot /var/www/redes4-5TX.com.ni
...
```bash
sudo mkdir -p /var/www/redes4-5TX.com.ni
echo "<h1>Grupo 4 — redes4-5TX.com.ni</h1><p>Servidor web operativo.</p>" | sudo tee /var/www/redes4-5TX.com.ni/index.html
sudo a2ensite grupo-4.conf
sudo systemctl reload apache2
```

**RHEL:** paquete `httpd`, config en `/etc/httpd/conf.d/grupo-4.conf`
(mismo VirtualHost pero usa `ErrorLog logs/grupo4_error.log`). No hace falta
`a2ensite`.
...
sudo mkdir -p /var/www/redes4-5TX.com.ni
echo "<h1>Grupo 4 — redes4-5TX.com.ni</h1><p>Servidor web operativo.</p>" | sudo tee /var/www/redes4-5TX.com.ni/index.html
sudo chcon -R -t httpd_sys_content_t /var/www/redes4-5TX.com.ni
sudo systemctl enable --now httpd
sudo firewall-cmd --add-service=http --permanent && sudo firewall-cmd --reload
```

**Verificacion:** `curl http://www.redes4-5TX.com.ni` debe mostrar el HTML del Grupo 4.
...
```bash
sudo useradd -m ftpgrupo4
echo "ftpgrupo4:redes4" | sudo chpasswd
...
prueba `ftp ftp.redes4-5TX.com.ni` con el usuario creado.
...
```
myhostname = mail.redes4-5TX.com.ni
mydomain = redes4-5TX.com.ni
myorigin = $mydomain
inet_interfaces = all
inet_protocols = ipv4
mydestination = $myhostname, localhost.$mydomain, localhost, $mydomain
mynetworks = 192.168.28.96/27, 127.0.0.0/8
...
**Verificacion:** `echo "Test" | mail -s "Prueba Grupo 4" root@localhost` y
...
| DNS directa | `/etc/bind/db.redes4-5TX.com.ni` | `/var/named/db.redes4-5TX.com.ni.zone` |
| DNS inversa | `/etc/bind/db.192.168.28` | `/var/named/192.168.28.zone` |
| Web      | `/etc/apache2/sites-available/grupo-4.conf` | `/etc/httpd/conf.d/grupo-4.conf` |
| Web raiz | `/var/www/redes4-5TX.com.ni/` | `/var/www/redes4-5TX.com.ni/` |
| FTP      | `/etc/vsftpd.conf` | `/etc/vsftpd/vsftpd.conf` |
| SMTP     | `/etc/postfix/main.cf` | `/etc/postfix/main.cf` |

## Checklist final

- [ ] `ip a` muestra 192.168.28.98/27
- [ ] DHCP entrega IPs en el rango 192.168.28.99 — 192.168.28.126
- [ ] `nslookup www.redes4-5TX.com.ni 127.0.0.1` devuelve 192.168.28.98
- [ ] `curl http://www.redes4-5TX.com.ni` responde
